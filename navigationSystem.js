...
function startNavigation(destinationName) {
    ...
    // Enable camera follow if available
    if (window.cameraFollowSystem && typeof window.cameraFollowSystem.enable === 'function') {
        console.log("🎥 Enabling camera follow for navigation");
        window.cameraFollowSystem.enable();
    }
...
function stopNavigation() {
    ...
    // Disable camera follow if available
    if (window.cameraFollowSystem && typeof window.cameraFollowSystem.disable === 'function') {
        console.log("🎥 Disabling camera follow");
        window.cameraFollowSystem.disable();
    }
    ...
    navUI.innerHTML = `
        ...
        <div style="display: flex; gap: 10px; margin-bottom: 10px;">
            <label style="display: flex; align-items: center; cursor: pointer;">
                <input type="checkbox" id="navigationCameraToggle" checked style="margin-right: 8px;">
                Camera Follow
            </label>
        </div>
...
// Add camera follow toggle listener
const cameraToggle = document.getElementById('navigationCameraToggle');
if (cameraToggle && window.cameraFollowSystem) {
    ...
    console.log("✅ Added camera follow toggle listener");
}
